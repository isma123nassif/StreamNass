...
// Detect the video quality from the stream's name/description so we can show a
// badge (4K / 1080p / …) instead of relying on the raw text. Ordered most- to
// least-specific; bare "HD"/"SD" are intentionally ignored (too many false hits).
const QUALITY_RULES = [
    ...
    [/\b480p\b/i, '480p'],
    [/\b360p\b/i, '360p'],
    [/\b(cam|hdcam|ts|telesync|hdts)\b/i, 'CAM'],
];
const detectQuality = (text) => {
    if (typeof text !== 'string' || text.length === 0) {
        return null;
    }
    for (let i = 0; i < QUALITY_RULES.length; i++) {
        if (QUALITY_RULES[i][0].test(text)) {
            return QUALITY_RULES[i][1];
        }
    }
    return null;
};
...
const Stream = ({ className, videoId, videoReleased, addonName, name, description, thumbnail, progress, deepLinks, ...props }) => {
    ...
    const quality = React.useMemo(() => {
        return detectQuality(`${name || ''} ${description || ''}`);
    }, [name, description]);
            ...
            <Button className={classnames(className, styles['stream-container'])} title={addonName} href={href} target={target} download={download} onClick={onClick} {...props}>
                <div className={styles['stream-text']}>
                    <div className={styles['info-container']}>
                        <div className={styles['stream-headline']}>
                            {
                                quality ?
                                    <div className={classnames(styles['quality-badge'], styles[`quality-${quality.toLowerCase()}`])}>{quality}</div>
                                    :
                                    null
                            }
                            {
    ...
    }, [thumbnail, progress, addonName, name, description, href, target, download, onClick, quality]);
